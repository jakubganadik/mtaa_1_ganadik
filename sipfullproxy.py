...
class UDPHandler(SocketServer.BaseRequestHandler):
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            ...
            if self.data[0] == "SIP/2.0 100 Trying":  # ----my part maybe it needs is
                glob_vars.start_call = True
            ...
            if rx_register.search(request_uri):
                ...
                try:
                    call_log("caller: " + self.data[11].split(":")[2].split("@")[0])
                    call_log("called: " + self.data[0].split(":")[1].split("@")[0])
                    call_log("at time: " + str(datetime.datetime.now()))
                except:

                    call_log("video started at: " + str(datetime.datetime.now()))
                ...
                glob_vars.start_call = False
                call_log("call ended at: " + str(datetime.datetime.now()))
                ...
                if glob_vars.start_call == True and self.data[0] == 'SIP/2.0 200 Ok':
                    call_log("call started at: " + str(datetime.datetime.now()))
                    glob_vars.start_call = False
                if self.data[0] == 'SIP/2.0 603 Decline':
                    call_log("call declined at: " + str(datetime.datetime.now()))
                    glob_vars.start_call = False
